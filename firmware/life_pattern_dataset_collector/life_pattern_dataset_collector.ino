// 생활 패턴 4-class 데이터 수집기 - presence_dataset_collector.ino와 완전히 같은 구조(AP 핫스팟 +
// 사진 촬영 웹페이지)이고, 라벨 입력창에 4개 클래스를 한 번에 넣어주는 버튼만 추가했다.
// 사실 원본 collector도 라벨을 자유 텍스트로 입력할 수 있어서 그대로 써도 되지만, 매번 타이핑하지
// 않도록 버튼으로 편의를 더했다.
//
// 연결: WiFi "XIAO_LIFEPATTERN" (pw vita1234), 브라우저에서 http://192.168.4.1 접속
// 사용법: 침대에 앉아서 "침대" 버튼 누른 뒤 버스트 캡처 → 책상에 앉아서 "책상" 버튼... 식으로
// 각 라벨당 최소 80~100장씩 모은 뒤(명세서 기준 400장/4클래스) ZIP으로 받아 Edge Impulse에 업로드한다.
#include <WiFi.h>
#include <WebServer.h>
...
<title>XIAO 생활 패턴 데이터 수집기</title>
<style>
...
button.preset{background:#6c4cf5;font-size:14px;padding:8px 12px}
...
<h2>XIAO 생활 패턴 데이터 수집기</h2>
<img id="cam" src="/jpg">
<div>
<button class="preset" onclick="setLabel('Bed_Activity')">🛏 침대</button>
<button class="preset" onclick="setLabel('Desk_Activity')">💻 책상</button>
<button class="preset" onclick="setLabel('Moving')">🚶 이동</button>
<button class="preset" onclick="setLabel('Out_of_Room')">🚪 외출</button>
</div>
<div>
<input id="label" placeholder="라벨" value="Bed_Activity">
...
<div id="count">0장 보관 중 - 라벨 버튼을 누르고 캡처하면 아래 갤러리에 임시 보관됩니다. X로 지우고, ZIP으로 한꺼번에 받으세요.</div>
<div id="gallery"></div>
<script>
let shots=[];  // {label, u8, url}
setInterval(()=>{cam.src='/jpg?t='+Date.now()},700);

function setLabel(v){ document.getElementById('label').value=v; }
...
function zipAll(){
  ...
  a.href=URL.createObjectURL(blob); a.download='life_pattern_dataset.zip'; a.click();
...
void setup() {
  ...
  WiFi.mode(WIFI_AP);
  WiFi.softAP("XIAO_LIFEPATTERN", "vita1234", 1);
