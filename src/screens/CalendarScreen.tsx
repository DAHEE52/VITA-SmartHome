...
//
// 일정은 CalendarContext를 통해 실제 백엔드(schedule_items 테이블)에 저장된다.
...
export default function CalendarScreen() {
  ...
  const { dailyItems, specialItems, addDailyItem, addSpecialItem, replaceDailyItems, replaceSpecialItems } =
    useCalendar();
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      ...
      <ScheduleEditModal
        ...
        onSave={(items) => replaceDailyItems(dailyItems, items)}
      />
      <ScheduleEditModal
        ...
        viewMonth={viewMonth}
        onClose={() => setEditingSection(null)}
        onSave={(items) => replaceSpecialItems(displayedSpecialItems, items)}
      />
      ...
      <AddScheduleModal
        ...
        onClose={() => setAddingSection(null)}
        onAdd={(item) => addSpecialItem(item)}
      />
    </SafeAreaView>
  );
}
