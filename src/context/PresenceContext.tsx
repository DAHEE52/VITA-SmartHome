...
// HomeSummaryContext(/home/summary를 한 곳에서만 폴링)의 presence 필드로 반영한다. 아직 카메라가
// 한 번도 값을 보낸 적이 없으면(presence: null) 서버가 판단 불가 상태이므로 이전 값을 그대로 유지한다.
...
export function PresenceProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if (summary?.presence != null) setIsHome(summary.presence);
  }, [summary]);
