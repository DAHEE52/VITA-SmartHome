...
//
// 주의: 이 방/기기 목록은 SmartHomeControlScreen이 실제로 보여주는(백엔드 `getRoomsStatus()`
// 기반) 방 목록과는 완전히 별개다. 이건 자동화/전기요금/에너지나무/화재감지 화면들이 아직
// 실제 하드웨어 없이도 동작을 시연할 수 있도록 만든 순수 시뮬레이션 데이터라서, 이름을
// `DemoRoomsContext`/`useDemoRooms`로 분리해뒀다 (실제 rooms API와 헷갈리지 않도록).
//
...
const DemoRoomsContext = createContext<DemoRoomsContextValue | null>(null);

export function DemoRoomsProvider({ children }: { children: ReactNode }) {
...
export function useDemoRooms() {
  const ctx = useContext(DemoRoomsContext);
  if (!ctx) throw new Error('useDemoRooms must be used within a DemoRoomsProvider');
  return ctx;
}
